...
export class ImageUploadService {
  ...
  async captureExpenseReceipt(source: CameraSource = CameraSource.Camera) {
    const imageData = await Camera.getPhoto({
      resultType: CameraResultType.Uri, // file-based data; provides best performance
      source, // automatically take a new photo with the camera
  ...
  async createUpdateExpense(expense: any) {
    ...
    if (expense.receipt.tempPath) {
      ...
      expense.receipt.tempPath = ""; // clear placeholder image
  ...
  async removeExpense(expense: any, position: number) {
